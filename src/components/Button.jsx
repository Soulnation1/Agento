...
const Button = ({ title, types, size, onClick, disabled, className }) => {
  const style = clsx(
    " text-[#ffffff] py-3 rounded",
    types === "danger" &&
      "bg-[#fff0f0] hover:bg-red-100 text-[#e04040] border border-[#f8d8d8] rounded-md ",
    types === "success" && "bg-green-500 hover:bg-green-600 text-white",
    types === "warning" && "bg-yellow-500 hover:bg-yellow-600 text-white",
    types === "common" &&
